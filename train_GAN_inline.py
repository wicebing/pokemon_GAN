...
def save_checkpoint(checkpoint_path, model, optimizer):
    ...
    torch.save(state, os.path.join('./checkpoint_out',checkpoint_path))
    name_='W_'+checkpoint_path
    torch.save(model,os.path.join('./checkpoint_out',name_))
    print('model saved to %s' % checkpoint_path)
    
def load_checkpoint(checkpoint_path, model, optimizer):
    state = torch.load(os.path.join('./checkpoint_out',checkpoint_path))
...
#                print('======== Train Discriminator =========')
                ...
                for param in GAN_G.G.parameters():
                    param.requires_grad = False                
                for param in GAN_G.D.parameters():
                    param.requires_grad = True    
                
                optimizer_GAN_G.zero_grad()
                z = z_creator(batch_size=batch_s).to(device1)
                
                output_f, output_r,fake_img = GAN_G(z,real_img)
...
#                print('======== Train Generator =========')
        ...
        if ep % plotep ==0:
            ...
            imshow(torchvision.utils.make_grid(fake_img[:36].detach().cpu(),6),
                   ep+0+int(startnum),outfile ='../output/GAN_out/')
            ...
            with torch.no_grad():       
                ...
                imshow(torchvision.utils.make_grid(fix_GAN[:36].detach().cpu(),6),
                      'GAN'+str(ep+0+int(startnum)),outfile ='../output/fix_out/')
